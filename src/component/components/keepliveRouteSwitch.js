...
import { Switch, matchPath, useHistory, withRouter } from 'react-router-dom';
...
const KeepSwitch = ({ children, withoutRoute = false, deep = true, ...props }) => {
			...
			<Cache {...props}>
				{(cacheProps) => {
					return withoutRoute ? (
						...
						<KeepliveRouterSwitch {...props} {...cacheProps} ishasRouterSwitch>
							{children}
	...
	return <KeepliveRouterSwitch {...props}>{children}</KeepliveRouterSwitch>;
};

export default useHistory ? withRouter(KeepSwitch) : KeepSwitch;
